...
public class NebulaGridExamplePage {
  ...
  public Grid createControl( Composite parent ) {
    initData();
  ...
  private Grid createGrid( Composite parent ) {
    Grid grid = new Grid( parent, SWT.V_SCROLL | SWT.H_SCROLL | SWT.BORDER );
...
//    grid.setLayoutData( tableLayoutData );
  ...
  private void createYearColumn( Grid grid, GridColumnGroup group, String text ) {
    GridColumn column = createColumn( grid, group, text, 110, SWT.RIGHT );
    column.setSummary( false );
  }

  private void createTotalColumn( Grid grid, GridColumnGroup columnGroup ) {
    GridColumn totalColumn
      = createColumn( grid, columnGroup, "Total (2006 - 2009)", 440, SWT.RIGHT );
